...
from os.path import abspath, exists, expanduser, expandvars


from fabric.api import abort, hide, local, puts, quiet, settings, warn


def readoptions(overrides):
    if exists("defaults.json"):
        defaults = loads(open("defaults.json", "r").read())
    else:
        defaults = {}

    if exists(".options.json"):
        useroptions = loads(open(".options.json", "r").read())
        if useroptions is not None:
            options = defaults
            options.update(useroptions, **overrides)
        else:
            options = defaults
...
def saveoptions(options):
    open(".options.json", "w").write(dumps(options, indent=4))
...
def pidof(name):
    with settings(hide("warnings"), warn_only=True):
        return local("pidof {0:s}".format(name), capture=True)


@requires("pgrep")
def pgrep(name):
    with settings(hide("warnings"), warn_only=True):
        return local("pgrep -f {0:s}".format(name), capture=True)
